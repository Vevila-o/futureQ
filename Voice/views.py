...
# 🌐 ✨ 引入 OpenAI 官方庫（100% 安全隔離，不留任何金鑰字串）
...
# 🎙️ ✨ 終極升級完全體：接收聊天室真實語音 ➡️ Whisper 轉文字 ➡️ OpenAI 智慧溫暖且限制3次的回應
@csrf_exempt
def upload_chat_voice(request):
    ...
    try:
        ...
        # === 階段二：推進問答階段與扣除次數（計算這是第幾次對話） ===
        chat_count += 1
        request.session['chat_count'] = chat_count
        remaining = 3 - chat_count


        # === 階段三：呼叫 OpenAI 進行智慧溫暖且籠統的回覆生成 ===
        # 🟢 修正後的安全寫法：純粹由本機 .env 讀取環境變數，絕不寫死金鑰字串，完美通過 GitHub 檢查！
        ...
        # 🧠 根據目前是「第幾次對話」，給 OpenAI 不同的劇情指引，實現 AI 三次回覆的層次感！
        if chat_count == 1:
            stage_instruction = "這是長輩今天的第一句話。請給出一句讚美或肯定，並引導長輩描述照片或今天的細節（例如：最喜歡哪個部分、和誰一起）。"
        elif chat_count == 2:
            stage_instruction = "這是長輩的第二句話（對答第二輪）。請順著他的話給予溫暖肯定，並試著拋出好回答的問題來深挖細節（例如：當時有沒有發生什麼有趣的事、那後來呢）。"
        else:
            stage_instruction = "這是長輩的最後一句話了（第三輪）。請給予極度溫暖、感性且圓滿的總結，感謝長輩今天的分享，並詢問他今天心情如何，作為今天日記的溫馨收尾。"

        # 🧠 核心 Prompt 咒語設定：控管 AI 必須給出溫暖、精簡、籠統且帶有小追問的回應
        system_prompt = (
            "你是一位溫柔、有耐心、專門陪伴高齡長輩的 AI 聊天助手。現在長輩剛剛錄製完一段生活日記，並對你說了一句話。\n"
            f"【目前對話階段指引】：{stage_instruction}\n\n"
            "請遵守以下鐵律進行回覆：\n"
            ...
            "4. 如果是第一或第二輪，最後必須拋出一個非常簡單、沒壓力的開放式小問題；如果是第三輪（最後一次），請感性收尾，絕對不要再問問題了。"
        )
        ...
        return JsonResponse({
            'status': 'success', 
            'user_text': whisper_text, # 長輩講的真實文字
            'reply': ai_reply,         # OpenAI 根據三輪節奏生成的智慧暖心回應！
            'remaining': remaining
        })
